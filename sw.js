...
messaging.onBackgroundMessage(function(payload) {
  ...
  var title  = data.title  || 'Hatzolah MC';
  var body   = data.body   || 'Tap to view';
  var tag    = data.tag    || 'hatzolah-update';
  var callId = data.callId || '';

  // Only play tone for new calls and call updates — not CE/education notifications
  var isCallNotif = tag.indexOf('hatzolah-call') !== -1 || tag.indexOf('hatzolah-update') !== -1 ||
                    tag.indexOf('hatzolah-assigned') !== -1 || tag.indexOf('hatzolah-als') !== -1 ||
  ...
  return self.registration.showNotification(title, {
    ...
    tag: tag,
    renotify: true,
    requireInteraction: tag.indexOf('hatzolah-call') !== -1,
...
self.addEventListener('message', function(e) {
  if (e.data && e.data.type === 'PLAY_TONE') {
    // Tone is played by the app window
  }
});
